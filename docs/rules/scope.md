...
2. フォアグラウンドアプリの選択テキストを取得し、初期テキストとして表示（選択なしの場合は空）
...
| PATH | 親プロセスから継承 |
| HOME | 親プロセスから継承 |
| KOECHO_SELECTION | パネル内の選択テキスト（未選択時は空） |
| KOECHO_SELECTION_START | パネル内の選択開始位置（先頭からの文字数） |
| KOECHO_SELECTION_END | パネル内の選択終了位置（先頭からの文字数） |
...
- 内蔵操作は prompt を必要とせず、確定時 auto-run の候補にはならない。
...
箇条書きに変換（LLM 連携の例）:
  # KOECHO_SELECTION にパネル内の選択テキストがある
...
- パネル開閉
- Auto-run スクリプト選択
- 認識言語切り替え（SpeechAnalyzer モード / ダウンロード済み言語 2+ の場合のみ）
- 最後の履歴をコピー
- 設定を開く
- 終了
...
- 音声入力オフモード: `voiceInputMode = .off` でエンジン起動・ボリュームダッキングをスキップ。キーボード入力のみで使用可能。Settings のエンジン選択で Off を選択して切り替え
...
- **モード（プリセット）**: ユーザー定義のモードごとにスクリプトのリストを切り替える。例:「議事録モード」「翻訳モード」など
