...
1. **監督起動**: main は `Agent` ツールで Opus subagent（risk-check 監督）を 1 体起動する。prompt には次を渡す:
   ...
   - 参照すべきパス: 該当する `docs/rules/`（mission / scope / architecture / constraints / principles）、`docs/specs/`（あれば）、`docs/decisions/`、`llm-wiki/`。
...
5. **main 側の責務**: 修正と、`docs/specs/` / `backlog/backlog.md` / `docs/decisions/` / `llm-wiki/` への同期判断は main 側で行う。
