...
- Checkpoints と対象を照合する際、必要に応じて `rules/` と関連 ADR（`decisions/`）を Read で読む。
- main で直接チェックを回さず、Opus 監督 subagent に隔離する fork 構造で実行する（Execution 参照）。監督・各 subagent は修正をせず、修正と同期判断は main に残す。

## Acceptance

- `LGTM` またはリスク一覧がある。
- リスクには影響、根拠、推奨対応がある。
- 必要な場合、更新すべき `rules/`, `backlog/backlog.md`, `decisions/`, `references/knowledge.md` が明確。
...
main で直接チェックを回さず、fork 構造で実行する。

1. **監督起動**: main は `Agent` ツールで Opus subagent（risk-check 監督）を 1 体起動する。prompt には次を渡す:
   - 対象（plan ファイルのパス / 未コミット差分 / commit range のいずれか）。
   - 参照すべきパス: 該当する `rules/`（mission / scope / architecture / constraints / principles）、`specs/`（あれば）、`decisions/`、`references/knowledge.md`。
   - 監督に Fable ではなく Opus を使うのは、main の context 隔離が目的で、修正と同期判断は main に残すため。
2. **観点クラスタへの分配**: 監督は下記 Checkpoints を観点クラスタに分け、subagent 2〜5 体（`model: sonnet` を必ず明示）に振り分けて並列起動する。クラスタ例:
   - (a) Mission / Scope
   - (b) Architecture / 依存方向
   - (c) ドメイン semantics（テキストライフサイクル / 永続化 / 権限・システム連携）
   - (d) 既知の落とし穴 + `references/knowledge.md` 照合
   - 対象が小さい場合は観点をまとめて体数を減らしてよい。
3. **各 subagent への指示**: 各 subagent には「ファイルパス・行番号つきの事実と、該当する Checkpoint のみ返す。推測・提案・『推奨事項』セクションは含めない（判断は監督・main 側でやる）」を必ず指示する。
4. **監督の統合**: 監督は各 subagent の結果を dedup し、🔴 MUST / 🟡 SHOULD / 🔵 NIT を付けて一覧に統合して返す。監督は修正を一切行わない。Koecho 固有の指摘がなければ「Koecho 固有の指摘なし（LGTM）」とだけ返す。
...
- 軽量・即応の音声入力という mission と矛盾しないか（`rules/mission.md`）。
- 非目標（`rules/scope.md` 外の機能）を混ぜていないか。
...
- 監督が統合した一覧を main に返す。日本語。指摘には 🔴 MUST / 🟡 SHOULD / 🔵 NIT を付け、`file:line` で該当箇所を引用する。
