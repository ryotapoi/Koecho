...
- **Constraints**:
  ...
  - 実装中に 1 commit として不自然だと分かったら、作業を広げず `change/plan.md` に戻るか、今回扱う 1 commit 単位へ切り直す。Goal 実行中の Change worker は事実を Goal main に返し、Goal main が切り直す。
...
- XcodeBuildMCP を優先する: ビルドは `build_macos`、アプリテストは `test_macos`（`-only-testing:KoechoTests` で UITests を除外）、実画面確認は `build_run_macos`。
- KoechoKit の SPM テストは `swift test --package-path Packages/KoechoKit`。
- ビルド・テスト・Preview・ドキュメント検索は MCP ツールを使い、Bash で `xcodebuild` を直接叩かない。
