...
- 起動直後にログヘッダの `session id:` を記録する。resume は `codex exec resume <session_id> -c sandbox_mode=workspace-write "<prompt>"` と session ID を明示する。`--last` は使わない（同一 Change 内に Implementer 用と Gatekeeper 用の codex セッションが並存し得るため、掴む対象が不定になる）。timeout した場合は破棄せず resume で継続する。
...
- 目視・実行でしか確定できない成果物（UI の見た目、CLI の出力等）は、証拠（レンダ画像・スクリーンショット・実行ログ等）の取得を義務化し、さらに codex 自身が画像閲覧ツールで証拠を開いて内容を確認し、確認した内容を最終メッセージで報告させる（codex exec はローカル PNG をパス指定で閲覧できる。resume 後も可。2026-07-13 実測）。証拠のパスは Gatekeeper への引き継ぎに含める
  ...
  - SwiftUI View の見た目変更では、実装エージェントに対象 View の `#Preview` 整備とビルド成功の報告を求める。PNG 証拠は実装エージェント側に取得手段がないため、提出義務の明示的な例外とし、Orchestrator が Apple Xcode MCP の `RenderPreview` で取得して目視確認する（`docs/rules/xcode-mcp.md` 参照）。この場合、PNG が無いことだけを理由に完了報告を証拠不足として扱わない。
  <!-- /slot -->
