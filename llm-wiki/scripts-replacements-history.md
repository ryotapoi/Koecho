...
- `Script` は custom と builtin を kind / feature ID で保存する。builtin は command/name を保存値として使わず、indent feature は 2 / 4 spaces を保存する。`ScriptSettings` は専用 flag により default builtin を一度だけ既存 scripts の後ろへ追加するため、削除後に復元しない。builtin は prompt 不可かつ auto-run 候補外。入力パネルの横並び strip は macOS 27 で ID ベースの drag-to-reorder を提供し、App target の局所 helper が完成配列を `ScriptSettings.scripts` へ渡す。設定 UI は builtin の feature / width / shared shortcut だけを bind し、`ScriptPresentation` が App target で SF Symbol を選ぶ。実行時は `ScriptExecutionService` が builtin を `BuiltinTextOperation` へ dispatch し、volatile を確定してから UTF-16 の全文・selection を変換する。builtin は `ScriptRunner` を通らず、App target が suppression 下で text view / AppState / voice insertion point を同じ結果へ同期する。
