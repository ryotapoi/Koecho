...
1. **Primary: Transcriber 再生成** — キーボード入力時に `DictationTranscriber` + `SpeechAnalyzer` + `AsyncStream` を再生成（オーディオエンジンは維持）。セグメントバッファがクリアされ、次の `didFinalize` が新しいバッファから開始される。Apple 内部実装への依存だが、実験で 70-100% → ほぼ 0% に改善を確認
