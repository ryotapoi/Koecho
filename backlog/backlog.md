...
### 修正進め方
- [x] 再現手順の確立（長文音声入力中に Ctrl+B でカーソル移動）
- [x] ログ仕込み: `didUpdateVolatile` で suppression 判定の入出力を記録
- [x] 原因特定後に修正（候補: `didUpdateVolatile` にも `stripOverlappingPrefix` 相当を適用）
- [x] ユーザー確認で再発しないことを検証
- [x] ADR 0016 の適用範囲注記（「キーボード入力時」→「ユーザー操作による割り込み時」）
