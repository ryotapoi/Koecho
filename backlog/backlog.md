...
- [ ] 選択範囲へ適用できる内蔵テキスト操作を追加する
  ...
  - 機能導入後の初回起動時に、この3機能を `[Decrease Indent] [Increase Indent] [Block Quote]` の順で既定登録する。`Increase Indent` / `Decrease Indent` の初期幅は 2 spaces とする。既存のユーザースクリプトは保持し、内蔵機能を重複登録しない
  - 初期登録済みであることを scripts 配列の空判定ではなく専用の永続フラグで管理する。ユーザーが既定項目を削除しても次回起動時に復元せず、必要なら設定画面から再登録できることを回帰テストする
  - 内蔵機能は既存のスクリプトボタン、ショートカット、並び順と同じ実行導線で扱う。選択範囲を持たない確定時 Auto-run の対象にはしない
  ...
  - `Block Quote` は選択範囲と交差する各行の先頭へ Markdown の `> ` を付ける。`>` のみを使う別モードは設けず、異なる形式が必要な場合はカスタムスクリプトで対応する
