...
この workflow は Koecho の Goal 手順の正本。実装作業の発火入口は `goal-workflow` skill とし、`goal-workflow` skill はこのファイルを読んで進める。

## ICAR

- **Intent**: `/goal` で指定された目的を、複数の 1 commit workflow に分割して完了まで進める。
- **Constraints**:
  - 実装作業はグローバル `goal-workflow` skill を入口にし、この workflow を正本として読む。プロジェクト内に `goal-workflow` skill は作らない。
  - `/goal` の呼び出し文は、原則として skill への参照と完了対象だけでよい。例: `/goal goal-workflow skill に従い、backlog/backlog.md の「v1.x」を完了して。`
  ...
  - 各 commit 内のレビューとは別に、Goal の commit range に対する Claude review を Goal 完了条件に含める（Claude Review 参照）。
- **Acceptance**:
  - Goal の目的が満たされている。
...
各 commit 内の `review.md` とは別に、Goal の commit range（`base..HEAD` など）を対象に Claude review を Goal 完了条件として実施する。Codex から見た別系統モデル（Claude）に Goal 差分全体を見せる。
