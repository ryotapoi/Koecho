...
- README / docs/rules / llm-wiki と実装の同期

上記に該当する場合は `project-risk-check` を使う。構造劣化リスクがある場合は `thermo-nuclear-code-quality-review` も必須で使う。

## How To Run

- Self-check: main で `git diff` を読み、acceptance と照合する。
- Standard: `change-review` を実行し、戻ってきた指摘を採否判断して反映する。
- Targeted / External supplement: 該当 skill（`project-risk-check`, `swiftui-pro`, `thermo-nuclear-code-quality-review`, `claude-review-request`）を使う。
- 複数該当するものは並列起動してよい。
