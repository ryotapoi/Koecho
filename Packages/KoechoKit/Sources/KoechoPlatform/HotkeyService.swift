...
public final class HotkeyService {
  ...
  public func start() {
    ...
    if globalMonitor == nil {
      logger.warning(
        "Failed to register global event monitor — Input Monitoring permission may be missing")
    }
  ...
  public func updateConfig(_ newConfig: HotkeyConfig) {
    ...
    cancelDoubleTapTimer()
    logger.info(
      "Hotkey config updated: \(newConfig.modifierKey.rawValue) \(newConfig.side.rawValue) \(newConfig.tapMode.rawValue)"
    )
  }
